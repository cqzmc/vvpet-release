...
<!-- 修复：fontSize 从 90 降到 40，height 从 300 升到 360，确保 vvPet 完整显示 -->
...
<!-- Tagline -->
<p align="center">
  <b>🐱 常驻桌宠 · 🤖 AI 助手 · 🎙️ 离线语音 · 🍅 番茄钟 · 📝 会议记录 · 🔤 划词翻译 · 📄 文档 OCR · 🧠 本地大模型 · 📧 邮件蒸馏 · 🏝️ 灵动岛 · 📸 全能截图</b>
</p>
...
- 文件上传与解析
- 区域截图识别，截图时自动隐藏主窗口与桌宠，完成后恢复上下文
- 截图标注支持文字、形状、画笔、马赛克等常用操作
- 本地 OCR 文字提取
...
- 本地文件搜索（需显式授权目录），文件名命中优先展示
- SQLite FTS5 全文索引兜底搜索文档内容，支持内容预览与文本行定位
- 拒绝静默扫描，隐私边界清晰
...
- 番茄钟专注计时与待办创建分离，减少事项和倒计时混淆
- 支持优先级、事件大小、完成 / 推迟 / 删除等待办操作
- 持久化保存，重启不丢失
- 桌宠状态联动反馈
...
- 桌面顶部轻量状态岛，承接截图、提醒、搜索、番茄钟等即时状态
- 待机时收起为圆形状态点，悬停或新消息到达时展开
- 点击可打开或收起主聊天框，减少工作流打断
...
| 📁 本地搜索 | 显式授权目录 + 本地 FTS5 索引 | 文件名优先，未命中或补充时搜索内容 |
