# vvPet
https://github.com/cqzmc/vvpet-release/blob/main/vvPet-poster.png
让 AI 住进你的桌面。
...
- 桌宠联动反馈：聊天、翻译、截图、会议、番茄钟、更新与通知会触发对应表情
- 桌宠浮动消息：主聊天框未打开时，在桌宠附近显示短消息提醒
- 宠物选择入口：内置像素猫，并预留本地宠物文件夹导入列表
- 微信文件传输助手：Windows 下通过系统 Edge app 窗口打开，避免 WebView2 白屏
- 离线语音能力：Vosk / Whisper 语音识别模型管理，Piper 文本转语音运行时与模型下载
- 朗读诊断：最近一次朗读会显示是否使用了 Piper、识别到的语言以及是否回退到系统语音
...
- API 请求默认直连你配置的模型服务
- API Key / Forvia Token 写入 Windows 凭据管理器，不写入前端构建产物或普通偏好文件
- 本地搜索必须先显式授权目录
